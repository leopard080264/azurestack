...
Download_ocserv(){
	...
	tar -xJf ocserv-0.12.1.tar.xz && cd ocserv-0.12.1
	./configure
	make
	make install
	cd .. && cd ..
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/leopard080264/azurestack/master/ocserv.conf"
...
Set_tcp_port(){
	...
	[[ -z "$set_tcp_port" ]] && set_tcp_port="8888"
